...
if __name__== '__main__':
    ...
    parser.add_argument('--job', type=str, dest='job_name', help="Name of the job module to be launch") #required=True,
    ...
    spark = SparkSession.builder.master("local[*]").appName('dataframe').getOrCreate() #args.job_name

    job_module = importlib.import_module('jobs.dataframe.job' ) # %args.job_name

    start = time.time()
    print('\n Spark session created %s \n' % (spark.sparkContext))
    job_module.main(spark)
    end = time.time()
